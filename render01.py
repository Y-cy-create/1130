...
def submit():
    ...
    #
    # data = string_clean(work)
    # data1 = jieba_cut(data, stop_words)
    # test_cv_category = cv_category_predict(data1)
    # dict_jobs = open_json_file('104_flask.json')
    # jobs_query = select_jobs(dict_jobs, str(test_cv_category), area, int(year), edu)
    # cv_BOW = turn_content_BOW(data1)
    # L = get_cv_jobs_similarity(cv_BOW, jobs_query)
